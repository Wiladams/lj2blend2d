...
struct BLFontFeatureSettingsView {
  ...
  //! container is in SSO mode only the number of items used will be overwritten by \ref blFontFeatureSettingsGetData().
...
public:
  //! \cond INTERNAL
  enum : uint32_t {
    //! Capacity of an SSO feature map.
    kSSOCapacity = 12,

    //! Mask containing an object marker and BLFontFeatureSettings object type, used to make it faster to get SSO size.
    //!
    //! To get a size from object info becomes `info & kSSOSizeMask >> BL_OBJECT_INFO_A_SHIFT`.
    kSSOObjectMask = (uint32_t(BL_OBJECT_INFO_MARKER_FLAG)) |
                     (uint32_t(BL_OBJECT_TYPE_FONT_FEATURE_SETTINGS) << BL_OBJECT_INFO_TYPE_SHIFT)
  };
  ...
  //!
  //! \note If the container is in SSO mode then all `BLFontFeatureItem` values will be created from the underlying
  //! SSO representation and `BLFontFeatureSettingsView::data` will point to `BLFontFeatureSettingsView::ssoData`. If the
  //! container is dynamic, `BLFontFeatureSettingsView::ssoData` won't be initialized and `BLFontFeatureSettingsView::data`
  //! will point to the container's data. This means that the view cannot outlive the container, and also during
  //! iterating the view the container cannot be modified as that would invalidate the view.
  ...
  //! Sets or inserts the given `key` to the map, associating the `key` with `value`.
  //!
  ...
  //!
  //! Nothing happens if the `key` is not in the map (\ref BL_SUCCESS is returned).
  ...
  //! Tests whether this map is equal to `other` - equality means that it has the same key/value pairs.
